...
export async function checkQuotedContext(image) {
  ...
  const defaultQuoted = await resolveMediaContext(imageEvent, "埋埋看看这个", { mediaRecognition: {} }, options)
  assert.equal(defaultQuoted.attachments.filter(item => item.visionEligible).length, 1, "默认提供引用图片，宿主重复图片只保留一份")
