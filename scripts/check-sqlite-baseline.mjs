...
try {
  ...
  assert.deepEqual(stateMigrations.map(row => row.id), ["001-baseline.sql", "002-tool-call-events.sql", "003-model-call-snapshots.sql"], "state database must apply the baseline, combined tool/runtime, and model snapshot migrations")
  ...
  assert.deepEqual(columnNames(state, "conversations"), ["conversation_key", "history_json", "last_seen_at", "expires_at"])
