...
const webProviderSourceFiles = [
  ...
  "web/client/features/providers/provider-import-dialog.js",
]
...
async function checkWebAndBoot() {
  ...
  assert(webProvidersSource.includes("ProviderEditor") && webProvidersSource.includes("removeSelectedProvider") && webProvidersSource.includes("pv-detail-actions"), "providers page should expose provider actions in the selected provider header")
  assert(webProvidersSource.includes("pv-header") && webProvidersSource.includes("provider-routing-compact") && webProvidersSource.includes("全局设置") && webProvidersSource.includes("requestRemoveProvider") && webProvidersSource.includes("requestRemoveModel") && webProvidersSource.includes("确认切换默认模型") && webProvidersSource.includes("confirmAction") && !webProvidersSource.includes("window.confirm"), "providers page should keep a compact service toolbar and modal confirmations for destructive and default-routing changes")
  ...
  assert(!webProvidersSource.includes("providerSectionItems") && !webProvidersSource.includes("activeSection") && webProvidersSource.includes("pv-shell") && !webProvidersSource.includes("provider-pane-tabs") && webProvidersSource.includes("查看路由详情"), "providers page should keep one service workspace with a provider list and detail pane")
  assert(webProvidersSource.includes("ProviderImportDialog") && webProvidersSource.includes("<Dialog") && !webProvidersSource.includes("showImportDrawer") && webProvidersSource.includes("existingIds") && webProvidersSource.includes("stream.value") && webProvidersSource.includes("全选可导入项"), "provider model import should open in a centered dialog, distinguish existing models from selectable new models, and configure streaming per imported purpose")
  assert(webProvidersSource.includes("is-default") && webProvidersSource.includes("pv-nav-star") && webProvidersSource.includes("默认回复"), "default models and their providers should expose consistent special states")
  ...
  assert(webProvidersSource.includes("embeddingCapability") && webProvidersSource.includes("向量模型参数") && webProvidersSource.includes("套用 BGE-M3 预设") && webProvidersSource.includes("pv-model-list"), "providers page should expose compact unified embedding model setup")
