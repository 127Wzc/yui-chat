...
import { confirmAction, request, saveConfigPatch, setDeveloperMode, store, toast } from "../../app/store/store.js"
import { asRecord, errorMessage, type UnknownRecord } from "../../shared/data.js"
import { renderRichContent } from "../../shared/rich-content.js"
import { modelMessagesImageCount } from "./log-media-display.js"
import { LogContextView } from "./log-context-display.js"
...
interface LogRun extends UnknownRecord {
  ...
  metadata?: UnknownRecord
  error_message?: string
  error_details?: UnknownRecord
  model_name?: string
  model_adapter?: string
  model_stream?: boolean | null
  model_names?: string[]
  model_adapters?: string[]
  model_streams?: Array<boolean | null>
  session_id?: string
}
...
interface MemoryExtractionLink {
  groupId: string
  windowId: string
}
...
function requestForDisplay(value: unknown): UnknownRecord {
  const source = asRecord(value)
  const { rawRequests: _rawRequests, responseMedia: _responseMedia, ...rest } = source
  return rest
}

function rawRequests(snapshot: ModelCallSnapshot | null | undefined): unknown[] {
  const value = asRecord(snapshot?.request).rawRequests
  return Array.isArray(value) ? value : []
}

function responseMedia(snapshot: ModelCallSnapshot | null | undefined): unknown[] {
  const value = asRecord(snapshot?.request).responseMedia
  return Array.isArray(value) ? value : []
}

function memoryExtractionLink(item: LogModelCall = {}): MemoryExtractionLink | null {
  const link = asRecord(modelMetadata(item).memoryExtraction)
  const groupId = String(link.groupId || "").trim()
  const windowId = String(link.windowId || "").trim()
  return groupId && windowId ? { groupId, windowId } : null
}

function memoryExtractionKey(item: LogModelCall = {}): string {
  const link = memoryExtractionLink(item)
  return link ? `${link.groupId}:${link.windowId}` : ""
}

function extractionItems(window: UnknownRecord | null | undefined): UnknownRecord[] {
  const items = asRecord(window).result
  return Array.isArray(items) ? items.map(item => asRecord(item)) : []
}

function extractionScopeLabel(item: UnknownRecord = {}): string {
  const scopeType = String(item.scopeType || "")
  if (scopeType === "group") return "群公共记忆"
  const target = String(item.targetName || "").trim() || (item.ownerId ? `QQ ${item.ownerId}` : "成员")
  return scopeType === "user" ? `${target}个人全局记忆` : `${target}个人群记忆`
}

function extractionActionLabel(value: unknown): string {
  const labels: Record<string, string> = { added: "新增", reinforced: "强化", updated: "更新", retracted: "撤回", ignored: "忽略" }
  const key = String(value || "")
  return labels[key] || key || "已处理"
}

function extractionEvidenceLabel(item: UnknownRecord = {}): string {
  const ids = Array.isArray(item.evidenceMessageIds) ? item.evidenceMessageIds.map(value => String(value || "").trim()).filter(Boolean) : []
  const count = Number(item.evidenceCount || ids.length || 0)
  if (!ids.length) return `证据 ${number(count)} 条`
  const preview = ids.slice(0, 3).join("、")
  return `证据 ${number(count)} 条 · ${preview}${ids.length > 3 ? "…" : ""}`
}
...
function protocolShortLabel(value: unknown): string {
  const key = String(value || "").trim()
  if (["openai-responses", "responses", "responses-api"].includes(key)) return "Responses"
  if (["openai-compatible", "qwen", "chatglm", "chat-completions", "chat"].includes(key)) return "Chat"
  if (["openai-images", "images", "image-generation"].includes(key)) return "Images"
  if (["claude", "claude-messages"].includes(key)) return "Claude"
  if (["gemini", "gemini-generate-content"].includes(key)) return "Gemini"
  if (["gemini-images", "gemini-image-generation"].includes(key)) return "Gemini Image"
  if (key === "mock") return "Mock"
  return key || "未知"
}

function streamLabel(value: unknown): string {
  if (value === true || value === "true" || value === 1) return "流式"
  if (value === false || value === "false" || value === 0) return "非流式"
  return "流式未知"
}

function modelNamesLabel(item: LogRun | LogModelCall = {}): string {
  const names = Array.isArray(item.model_names)
    ? item.model_names.map(value => String(value || "").trim()).filter(Boolean)
    : []
  if (names.length) return names.join("、")
  return String(item.model_name || item.model_identifier || "未知模型")
}

function modelProtocolsLabel(item: LogRun | LogModelCall = {}): string {
  const adapters = Array.isArray(item.model_adapters)
    ? item.model_adapters.map(value => String(value || "").trim()).filter(Boolean)
    : []
  if (adapters.length) return [...new Set(adapters.map(protocolShortLabel))].join("/")
  return protocolShortLabel(item.protocol || item.adapter)
}

function modelStreamsLabel(item: LogRun | LogModelCall = {}): string {
  const values = Array.isArray(item.model_streams) ? item.model_streams : [item.model_stream]
  const labels = [...new Set(values.map(streamLabel))]
  return labels.join("/") || "流式未知"
}
...
function messagePreview(value: unknown, limit = 72): string {
...
export const LogsTab = {
  ...
  setup() {
    ...
    const memoryExtractionStates = reactive<Record<string, MemoryExtractionState>>({})
    ...
    function extractionState(item: LogModelCall | null | undefined): MemoryExtractionState | null {
      const key = memoryExtractionKey(item || {})
      return key ? memoryExtractionStates[key] || null : null
    }

    async function loadExtractionResult(item: LogModelCall | null | undefined): Promise<void> {
      if (!item) return
      const link = memoryExtractionLink(item)
      const key = memoryExtractionKey(item)
      if (!link || !key) return
      const current = memoryExtractionStates[key]
      if (current?.loading || current?.window) return
      memoryExtractionStates[key] = { loading: true, error: "", window: null }
      try {
        const response = await request(`/api/memory/captures/group/${encodeURIComponent(link.groupId)}/windows/${encodeURIComponent(link.windowId)}?includeMessages=false`)
        const detail = asRecord(asRecord(response).detail)
        memoryExtractionStates[key] = { loading: false, error: "", window: asRecord(detail.window) }
      } catch (error) {
        memoryExtractionStates[key] = { loading: false, error: errorMessage(error), window: null }
      }
    }

    async function loadExtractionResults(items: LogModelCall[] = []): Promise<void> {
      await Promise.all(items.map(item => loadExtractionResult(item)))
    }
    ...
    async function openRun(row: LogRun) {
      try {
        ...
        void loadExtractionResults(state.detail.modelCalls || [])
    ...
    async function openModelDetailById(id: unknown) {
      ...
      try {
        ...
        void loadExtractionResult(modelDetail.value.modelCall)
      } catch (error) {
    ...
    function toggleDeveloperMode(value: unknown): void {
      const enabled = Boolean(value)
      setDeveloperMode(enabled)
      // 直接从模型详情切换时，自动切到请求分区，让开关变化立即可见。
      if (enabled) modelDetailTab.value = "request"
    }
    ...
    async function loadConversationTurn(item: ConversationItem | string) {
      ...
      if (cached) {
        ...
        void loadExtractionResults(cached.modelCalls || [])
      ...
      try {
        ...
        void loadExtractionResults(detail.modelCalls || [])
      } catch (error) {
    ...
    const timeline = computed(() => {
      ...
      const models = modelCalls.map(item => ({
        ...
        kind: "模型",
        kindKey: "model",
        label: item.model_name || item.model_identifier || "模型",
        detail: `${purposeLabel(item.purpose)} · ${modelStopLabel(item.stop_reason)} · 提供工具 ${availableTools(item)} · 本地调用 ${responseToolCalls(item)} · 托管调用 ${hostedToolCalls(item).length}${responsesRecoveryLabel(item) ? ` · ${responsesRecoveryLabel(item)}` : ""}${item.parent_tool_id ? ` · ${lineageLabel(item.parent_tool_id)}` : ""}`,
    ...
    return {
      ...
      number, time, duration, categoryLabel, deliveryLabel, statusLabel, modelStopLabel, protocolLabel, protocolShortLabel, streamLabel, modelNamesLabel, modelProtocolsLabel, modelStreamsLabel, shortId, messagePreview, hostedToolCalls, responsesRecoveryLabel, scopeLabel, sourceLabel, purposeLabel, lineageLabel, modelRouteDetail, availableTools, renderRichContent, refresh, resetAndRefresh, resetFilters, applyFilters,
      conversationScope, conversationTitle, promptPreview, toolGroupsForModel, unlinkedConversationTools,
      requestForDisplay, rawRequests, responseMedia,
    ...
    <div class="stack logs-page">
      ...
      <Panel title="运行链路日志" icon="list">
        ...
        <div class="logs-filter-scroll">
          <div class="logs-filter-row">
            ...
            <Field class="logs-filter-field" v-model="state.filters.purpose" label="用途标签" placeholder="chat / image-generation / embedding" />
        ...
        <div class="table-wrap logs-run-table"><table class="data-table"><thead><tr><th>开始时间</th><th>来源 / 用途</th><th>模型 / 协议</th><th>状态</th><th class="num">模型调用</th><th class="num">工具事件</th><th class="num">Token</th><th>用户 / 会话</th><th class="col-actions">操作</th></tr></thead><tbody><tr v-for="row in state.runs" :key="row.id"><td class="muted tiny">{{ time(row.started_at) }}</td><td><div class="cell-title">{{ sourceLabel(row.source) }}</div><span class="muted tiny">{{ purposeLabel(row.purpose) }}</span></td><td class="logs-model-cell"><div class="cell-title" :title="modelNamesLabel(row)">{{ modelNamesLabel(row) }}</div><span class="muted tiny">{{ modelProtocolsLabel(row) }} · {{ modelStreamsLabel(row) }}</span></td><td><span class="badge" :class="row.status === 'ok' ? 'on' : (row.status === 'error' ? 'risk-high' : 'risk-medium')">{{ statusLabel(row.status) }}</span><small v-if="row.error_message" class="table-subvalue text-danger logs-inline-message" :title="row.error_message">{{ messagePreview(row.error_message) }}</small></td><td class="num">{{ number(row.model_calls) }}</td><td class="num">{{ number(row.tool_calls) }}<small v-if="row.failed_tools" class="table-subvalue">失败 {{ number(row.failed_tools) }}</small></td><td class="num">{{ number(row.total_tokens) }}</td><td class="muted tiny logs-session-cell"><span>{{ scopeLabel(row) }}</span><span v-if="row.session_id || row.conversation_key" class="logs-session-short" :title="row.session_id || row.conversation_key">sessionId {{ shortId(row.session_id || row.conversation_key) }}</span></td><td class="col-actions"><button class="btn small outline" type="button" @click="openRun(row)">查看详情</button></td></tr></tbody></table><div v-if="!state.runs.length" class="logs-empty"><Icon name="activity" :size="22" /><b>没有匹配的日志</b><span>调整筛选条件后再试一次。</span></div></div>
      ...
      <SideDrawer :open="drawerOpen" title="运行链路详情" subtitle="按模型请求、工具轮次和结束状态展示完整链路" icon="activity" width="720px" @close="drawerOpen = false">
        <div v-if="state.detail" class="logs-detail">
          <div class="detail-summary"><div><span class="badge" :class="state.detail.run.status === 'ok' ? 'on' : 'risk-medium'">{{ statusLabel(state.detail.run.status) }}</span><span v-if="state.detail.run.model_name" class="badge">{{ modelNamesLabel(state.detail.run) }}</span><span v-if="state.detail.run.model_name" class="badge">{{ modelProtocolsLabel(state.detail.run) }}</span><span v-if="state.detail.run.model_name" class="badge">{{ modelStreamsLabel(state.detail.run) }}</span><span class="muted">{{ time(state.detail.run.started_at) }} · 总耗时 {{ duration(state.detail.run.duration_ms) }}</span><span v-if="state.detail.run.session_id || state.detail.run.conversation_key" class="muted tiny logs-session-short" :title="state.detail.run.session_id || state.detail.run.conversation_key">sessionId {{ shortId(state.detail.run.session_id || state.detail.run.conversation_key) }}</span><span v-if="state.detail.run.error_message" class="text-danger tiny logs-inline-message" :title="state.detail.run.error_message">失败：{{ messagePreview(state.detail.run.error_message) }}</span></div><button v-if="state.detail.run.conversation_key" class="btn small outline" type="button" @click="openConversation"><Icon name="message" :size="13" />查看完整会话</button></div><JsonBlock v-if="store.developerMode && state.detail.run.error_details && Object.keys(state.detail.run.error_details).length" title="运行失败诊断详情" :value="state.detail.run.error_details" :open="true" />
          ...
          <details v-if="state.detail.run.response_text" class="logs-run-response"><summary>查看最终模型回复（{{ number(state.detail.run.response_text.length) }} 字符）</summary><div class="logs-rich-content" v-html="renderRichContent(state.detail.run.response_text)"></div><details v-if="store.developerMode" class="logs-raw-content"><summary>原始回复</summary><pre>{{ state.detail.run.response_text }}</pre></details></details>
      ...
      <SideDrawer :open="modelDetailOpen" title="模型请求详情" subtitle="本次请求的上下文与注入工具按需加载；内容已脱敏，超出上限时会标记截断" icon="cpu" width="1080px" modal @close="modelDetailOpen = false">
        <div class="logs-detail-window">
          ...
          <template v-else-if="modelDetail?.modelCall">
            <div class="logs-detail-window-meta"><div><span class="badge" :class="modelDetail.modelCall.status === 'ok' ? 'on' : 'risk-medium'">{{ statusLabel(modelDetail.modelCall.status) }}</span><span class="badge">{{ protocolShortLabel(modelDetail.modelCall.protocol || modelDetail.modelCall.adapter) }}</span><span class="badge">{{ streamLabel(modelDetail.modelCall.stream) }}</span><span class="muted">第 {{ modelDetail.modelCall.sequence || '-' }} 次 · {{ modelDetail.modelCall.model_name || modelDetail.modelCall.model_identifier || '-' }} · {{ purposeLabel(modelDetail.modelCall.purpose) }}</span><span v-if="modelDetail.modelCall.session_id || modelDetail.modelCall.conversation_key" class="muted tiny logs-session-short" :title="modelDetail.modelCall.session_id || modelDetail.modelCall.conversation_key">sessionId {{ shortId(modelDetail.modelCall.session_id || modelDetail.modelCall.conversation_key) }}</span></div><div class="row"><label class="heading-switch"><Switch :model-value="store.developerMode" @update:model-value="toggleDeveloperMode" /><span>{{ store.developerMode ? '开发者模式已开启' : '开发者模式' }}</span><HelpTip tip="开启后显示适配器实际发送的脱敏请求体和失败诊断；提示会浮在抽屉外，不会被滚动区域裁切。" /></label><span class="muted tiny">{{ time(modelDetail.modelCall.started_at) }} · {{ duration(modelDetail.modelCall.duration_ms) }}</span></div></div>
            <p v-if="modelDetail.snapshot?.truncated" class="logs-silent-note warning">快照已截断，以下只展示保留下来的内容。</p>
            ...
            <nav class="logs-window-tabs" aria-label="模型请求详情分区"><button type="button" :class="{ active: modelDetailTab === 'context' }" @click="modelDetailTab = 'context'">上下文 {{ number(modelDetail.snapshot?.message_count || 0) }}</button><button type="button" :class="{ active: modelDetailTab === 'tools' }" @click="modelDetailTab = 'tools'">可用工具 {{ number(modelDetail.snapshot?.tool_count || 0) }}</button><button type="button" :class="{ active: modelDetailTab === 'request' }" @click="modelDetailTab = 'request'">请求与回复</button></nav>
            <section v-if="modelDetailTab === 'context'" class="logs-window-section">
              <p v-if="modelDetail.snapshot" class="logs-context-note">{{ number(modelDetail.snapshot.context_chars) }} 字符<span v-if="modelMessagesImageCount(modelDetail.snapshot.messages)"> · {{ number(modelMessagesImageCount(modelDetail.snapshot.messages)) }} 张图片</span> · 已脱敏 · Token 为估算值</p>
              <LogContextView :key="modelDetail.modelCall.id" :snapshot="modelDetail.snapshot" :developer-mode="store.developerMode" />
            </section>
            <section v-else-if="modelDetailTab === 'tools'" class="logs-window-section"><p v-if="modelDetail.snapshot" class="logs-context-note">定义共 {{ number(modelDetail.snapshot.tool_chars) }} 字符 · 可用工具不代表已实际调用，Skill 指令在上下文中查看。</p><div v-if="modelDetail.snapshot?.tools?.length" class="logs-tool-catalog"><article v-for="(tool, index) in modelDetail.snapshot.tools" :key="'snapshot-tool-' + index" class="logs-snapshot-tool"><header><span class="logs-tool-number">#{{ index + 1 }}</span><b>{{ toolName(tool) }}</b><span v-if="tool.type" class="badge">{{ tool.type }}</span><span v-if="tool.defer_loading" class="badge on">延期加载</span><span v-if="tool.category" class="badge">{{ categoryLabel(tool.category) }}</span><span v-if="tool.source" class="badge">{{ tool.source }}</span><span v-if="tool.risk" class="badge risk-medium">{{ tool.risk }}</span></header><p v-if="toolDescription(tool)" class="muted">{{ toolDescription(tool) }}</p><details class="logs-inline-json"><summary>查看参数或配置</summary><pre>{{ pretty(toolParameters(tool)) }}</pre></details></article></div><p v-else class="logs-empty compact"><Icon name="wrench" :size="20" /><span>本次调用没有向模型注入工具。</span></p></section>
            <section v-else class="logs-window-section logs-request-section">
            <section v-if="store.developerMode" class="logs-developer-panel">
              <div class="logs-developer-panel-head"><div><b>开发者诊断</b><span class="muted tiny">已开启；只展示脱敏且限长的数据</span></div><span class="badge" :class="rawRequests(modelDetail.snapshot).length ? 'on' : 'risk-medium'">{{ rawRequests(modelDetail.snapshot).length ? '原始请求可见' : '无原始快照' }}</span></div>
              <JsonBlock v-if="rawRequests(modelDetail.snapshot).length" title="原始请求结构（已脱敏）" :value="rawRequests(modelDetail.snapshot)" :open="true" />
              <p v-else class="logs-silent-note">这条调用没有可显示的原始请求快照，可能是旧日志、非模型请求，或请求构建前采集失败。</p>
            </section>
              <JsonBlock title="请求元数据" :value="requestForDisplay(modelDetail.snapshot?.request || {})" /><JsonBlock v-if="hostedToolCalls(modelDetail.modelCall).length" title="OpenAI 托管工具执行摘要" :value="hostedToolCalls(modelDetail.modelCall)" :open="true" /><details v-if="responseMedia(modelDetail.snapshot).length" class="logs-run-response"><summary>查看生成图片（{{ number(responseMedia(modelDetail.snapshot).length) }} 张缩略图）</summary><div class="logs-rich-content" v-html="renderRichContent(responseMedia(modelDetail.snapshot))"></div></details><details v-if="modelDetail.modelCall.response_text" class="logs-run-response" open><summary>模型回复（{{ number(modelDetail.modelCall.response_text.length) }} 字符）</summary><div class="logs-rich-content" v-html="renderRichContent(modelDetail.modelCall.response_text)"></div><details v-if="store.developerMode" class="logs-raw-content"><summary>原始回复</summary><pre>{{ modelDetail.modelCall.response_text }}</pre></details></details><JsonBlock title="模型调用元数据" :value="modelDetail.modelCall.metadata || {}" /><p v-if="modelDetail.modelCall.error_message" class="logs-tool-error logs-inline-message" :title="modelDetail.modelCall.error_message">失败：{{ messagePreview(modelDetail.modelCall.error_message) }}</p><JsonBlock v-if="store.developerMode && modelDetail.modelCall.error_details && Object.keys(modelDetail.modelCall.error_details).length" title="失败诊断详情" :value="modelDetail.modelCall.error_details" :open="true" /></section>
          </template>
            ...
            <div v-if="conversationDetail.items?.length" class="logs-session-workbench">
              ...
              <section class="logs-turn-workspace">
                ...
                <template v-else-if="conversationTurnDetail?.run">
                  <div class="logs-turn-head"><div><b>{{ promptPreview(conversationTurnDetail.run.prompt_text) }}</b><span class="muted tiny">{{ time(conversationTurnDetail.run.started_at) }} · {{ statusLabel(conversationTurnDetail.run.status) }}<span v-if="conversationTurnDetail.run.session_id || conversationTurnDetail.run.conversation_key"> · sessionId {{ shortId(conversationTurnDetail.run.session_id || conversationTurnDetail.run.conversation_key) }}</span></span></div><span class="muted tiny">{{ number(conversationTurnDetail.run.total_tokens) }} Token · {{ number(conversationTurnDetail.run.tool_calls) }} 条工具事件</span></div>
                  <div class="logs-conversation-grid">
                    <section class="logs-message-card user"><b>用户提问</b><div class="logs-rich-content" v-html="renderRichContent(conversationTurnDetail.run.prompt_text || '这轮没有保存提问正文（可能是旧日志或非对话任务）。')"></div><details v-if="store.developerMode && conversationTurnDetail.run.prompt_text" class="logs-raw-content"><summary>原始提问</summary><pre>{{ conversationTurnDetail.run.prompt_text }}</pre></details></section>
                    <section class="logs-message-card assistant"><b>模型回复</b><div class="logs-rich-content" v-html="renderRichContent(conversationTurnDetail.run.response_text || '这轮没有文本回复，可能以工具或静默结果结束。')"></div><details v-if="store.developerMode && conversationTurnDetail.run.response_text" class="logs-raw-content"><summary>原始回复</summary><pre>{{ conversationTurnDetail.run.response_text }}</pre></details></section>
                  </div>
                        ...
                        <section v-for="group in toolGroupsForModel(model.id)" :key="'tool-round-' + model.id + '-' + group.round" class="logs-round-tool-group">
                          ...
                          <div class="logs-round-tool-list">
                            <article v-for="tool in group.items" :key="tool.id" class="logs-round-tool">
                              ...
                              <details v-if="tool.result_text" class="logs-tool-result"><summary>{{ tool.source === 'openai-hosted' ? '查看上游原始返回' : '查看返回结果' }}（{{ number(tool.result_chars || 0) }} 字符）</summary><div class="logs-rich-content" v-html="renderRichContent(tool.result_text)"></div><details v-if="store.developerMode" class="logs-raw-content"><summary>原始返回</summary><pre>{{ tool.result_text }}</pre></details></details>
                              <p v-if="tool.error_message" class="logs-tool-error logs-inline-message" :title="tool.error_message">失败：{{ messagePreview(tool.error_message) }}</p>
                            </article>
                          </div>
                        </section>
                      </div>
