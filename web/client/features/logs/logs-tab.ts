    ...
    <div class="stack logs-page">
      ...
      <SideDrawer :open="drawerOpen" title="运行链路详情" subtitle="按模型请求、工具轮次和结束状态展示完整链路" icon="activity" width="720px" @close="drawerOpen = false">
        <div v-if="state.detail" class="logs-detail">
          <div class="detail-summary"><div><span class="badge" :class="state.detail.run.status === 'ok' ? 'on' : 'risk-medium'">{{ statusLabel(state.detail.run.status) }}</span><span class="muted">{{ time(state.detail.run.started_at) }} · 总耗时 {{ duration(state.detail.run.duration_ms) }}</span><span v-if="state.detail.run.error_message" class="text-danger tiny">失败 message：{{ state.detail.run.error_message }}</span></div><button v-if="state.detail.run.conversation_key" class="btn small outline" type="button" @click="openConversation"><Icon name="message" :size="13" />查看完整会话</button></div><JsonBlock v-if="store.developerMode && state.detail.run.error_details && Object.keys(state.detail.run.error_details).length" title="运行失败诊断详情" :value="state.detail.run.error_details" :open="true" />
      ...
      <SideDrawer :open="modelDetailOpen" title="模型请求详情" subtitle="本次请求的上下文与注入工具按需加载；内容已脱敏，超出上限时会标记截断" icon="cpu" width="1080px" modal @close="modelDetailOpen = false">
        <div class="logs-detail-window">
          ...
          <template v-else-if="modelDetail?.modelCall">
            <div class="logs-detail-window-meta"><div><span class="badge" :class="modelDetail.modelCall.status === 'ok' ? 'on' : 'risk-medium'">{{ statusLabel(modelDetail.modelCall.status) }}</span><span class="badge">{{ protocolLabel(modelDetail.modelCall.adapter) }}</span><span class="muted">第 {{ modelDetail.modelCall.sequence || '-' }} 次 · {{ modelDetail.modelCall.model_name || '-' }} · {{ purposeLabel(modelDetail.modelCall.purpose) }}</span></div><div class="row"><label class="heading-switch" data-tip="开启后显示适配器实际发送的脱敏请求体"><Switch :model-value="store.developerMode" @update:model-value="setDeveloperMode" /><span>{{ store.developerMode ? '开发者模式已开启' : '开发者模式' }}</span></label><span class="muted tiny">{{ time(modelDetail.modelCall.started_at) }} · {{ duration(modelDetail.modelCall.duration_ms) }}</span></div></div>
            ...
            <section v-else class="logs-window-section logs-request-section"><JsonBlock title="请求元数据" :value="requestForDisplay(modelDetail.snapshot?.request || {})" :open="true" /><JsonBlock v-if="store.developerMode && rawRequests(modelDetail.snapshot).length" title="原始请求结构（已脱敏）" :value="rawRequests(modelDetail.snapshot)" :open="true" /><p v-else-if="!store.developerMode && rawRequests(modelDetail.snapshot).length" class="logs-silent-note">已采集适配器原始请求结构；开启开发者模式后查看（鉴权信息已脱敏并限长）。</p><JsonBlock v-if="hostedToolCalls(modelDetail.modelCall).length" title="OpenAI 托管工具执行摘要" :value="hostedToolCalls(modelDetail.modelCall)" :open="true" /><details v-if="modelDetail.modelCall.response_text" class="logs-run-response" open><summary>模型回复（{{ number(modelDetail.modelCall.response_text.length) }} 字符）</summary><pre>{{ modelDetail.modelCall.response_text }}</pre></details><JsonBlock title="模型调用元数据" :value="modelDetail.modelCall.metadata || {}" /><p v-if="modelDetail.modelCall.error_message" class="logs-tool-error">失败 message：{{ modelDetail.modelCall.error_message }}</p><JsonBlock v-if="store.developerMode && modelDetail.modelCall.error_details && Object.keys(modelDetail.modelCall.error_details).length" title="失败诊断详情" :value="modelDetail.modelCall.error_details" :open="true" /></section>
          </template>
