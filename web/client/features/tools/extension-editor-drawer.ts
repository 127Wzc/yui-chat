    ...
    >
      ...
      <template v-else>
        ...
        <section v-else-if="customBuilder.step === 'resources'" class="custom-builder-pane">
          ...
          <div v-for="(row, index) in customBuilder.resources" :key="index" class="custom-builder-row"><Field label="别名" v-model="row.alias" placeholder="system-status" /><Field label="资源引用" v-model="row.reference" placeholder="yunzai:plugins/system/status.js" /><button class="icon-btn" type="button" title="浏览项目文件" @click="$emit('browse-resource', index)"><Icon name="folder" :size="15" /></button><button class="icon-btn danger" type="button" data-tip="移除资源" @click="customBuilder.resources.splice(index, 1)"><Icon name="trash" :size="15" /></button></div>
          <p v-if="!customBuilder.resources.length" class="muted small">还未引入资源。普通工具不需要这一步；需要复用 Bot 框架或其他插件时再添加。</p>
          <div class="row"><button class="btn small outline" type="button" @click="customBuilder.resources.push({ alias: '', reference: '' })"><Icon name="plus" :size="14" />添加资源</button><button class="btn small outline" type="button" @click="$emit('browse-resource', -1)"><Icon name="folder" :size="14" />浏览项目文件</button><span class="muted tiny">支持 <code>plugin:插件名/路径</code> 与 <code>yunzai:路径</code>；越界路径会被拒绝。</span></div>
        </section>
