...
已注入的 MCP 适配器在传输异常时仍留在统一 Tool Registry 中；后续触发会按服务去重地重新连接并重新发现工具。首次启动就连接失败时，名单或工具策略中已经给出名称的工具会先以“待恢复”适配器保留；全开放模式则由后续模型工具列表、诊断或日常定格入口主动重试发现。只读工具在连接异常且请求尚未得到结果时最多安全重试一次，写入或结果不确定的工具只返回异常，不自动重复副作用。明确关闭 MCP 总开关、服务开关或移除开放名单时，才会清理对应的运行工具和缓存。
