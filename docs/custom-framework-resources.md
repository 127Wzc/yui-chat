...
Custom 工具编辑器也提供项目文件选择器。在“框架资源”中点击“浏览项目文件”，选择后只填入资源别名和引用；模块仍需由 Custom Tool 包装其参数、权限与执行结果。工具详情和测试参数支持“创建动作”，动作复制始终共享该工具，不扩大其资源声明范围。

## 直接作为源码动作

动作中心的“项目源码”可直接声明单个文件并调用导出函数或插件类方法，也可读取 JSON / 文本资源。绑定仍通过 `frameworkResources` 与 `createFrameworkResourceAccess`，不拼接绝对路径，不在选择、保存或预览时导入模块。函数调用约定为 `(args, context)`；插件方法约定为 `new ExportedClass()` 后设置 `this.e`，再以 `(e, args, context)` 调用所选方法。需要其他构造参数或生命周期初始化的模块，继续用 Custom Tool 包装。

源码动作只保存引用、方法名和参数，复制预设后共享同一源码。结果回复设置独立于实现，可提取 JSON 字段，或者把返回的 `{ parts: [...] }`、`{ chain: [...] }` 和消息段数组交给 `message_send`，目标固定为当前会话。
