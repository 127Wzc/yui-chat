...
const builtinToolDisplayNamesZh: Record<string, string> = {
  ...
  music_play: "音乐播放", message_send: "消息发送", generate_image: "图片生成", query_userinfo: "查询用户信息", block_user: "拉黑用户",
...
export function isToolEnabledByConfig(config: unknown = {}, tool: unknown = {}): boolean {
  ...
  if (text(common.source) === "custom" && text(toolProvenance(tool).packageId)) return true
